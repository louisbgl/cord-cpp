...
int main() {
    ...
    // for string fields, min()/max() constrain the length of the value
    // username must be between 3 and 32 characters
    schema.add<std::string>("username").required().min(3).max(32);
    ...
    // for vector fields, min()/max() constrain the number of elements
    // at least 1 tag, at most 5
    schema.add<std::vector<std::string>>("tags").default_({}).min(1).max(5);
    ...
    std::cout << "username:   " << result.get("username").as<std::string>() << std::endl;
    std::cout << "ratio:      " << result.get("ratio").as<float>() << std::endl;
    std::cout << "log_level:  " << result.get("log_level").as<std::string>() << std::endl;
    std::cout << "tags:       ";
    for (const auto& tag : result.get("tags").as<std::vector<std::string>>())
        std::cout << tag << " ";
    std::cout << std::endl;
